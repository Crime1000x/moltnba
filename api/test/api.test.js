...
describe('Auth Utils', () => {
  test('generateApiKey creates valid key', () => {
    ...
    assert(key.startsWith('polysportsclaw_'), 'Should have correct prefix');
    assertEqual(key.length, 73, 'Should have correct length');
  });

  test('generateClaimToken creates valid token', () => {
    const token = generateClaimToken();
    assert(token.startsWith('polysportsclaw_claim_'), 'Should have correct prefix');
  });
  ...
  test('validateApiKey rejects invalid key', () => {
    ...
    assert(!validateApiKey('polysportsclaw_short'), 'Should reject short key');
  });

  test('extractToken extracts from Bearer header', () => {
    const token = extractToken('Bearer polysportsclaw_test123');
    assertEqual(token, 'polysportsclaw_test123');
  });
...
describe('Config', () => {
  test('config loads without error', () => {
    ...
    assert(config.polysportsclaw.tokenPrefix, 'Should have token prefix');
  });
});
